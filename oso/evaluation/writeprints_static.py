"""Writeprints-Static feature extractor.

The attribution model the paper's Delta Acc. column is measured against: the
same stylometric feature set Mutant-X (Mahmood et al., 2019) attacks, so a drop
in its accuracy is what "style concealment" means in Table 1.

Vendored from Avengers-Ensemble (https://github.com/Haroon96/Avengers-Ensemble),
with two changes: resource files are resolved inside this package, and the two
utility dependencies (keras' text_to_word_sequence, nltk's FreqDist) are
replaced by the small local equivalents below so that installing OSO does not
pull in TensorFlow.
"""
...
_RESOURCE_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), "writeprints_resources")
...
class _TextUtils:
    """Stand-in for keras.preprocessing.text, which is all this module used it for."""
    ...
    def text_to_word_sequence(input_text, filters='!"#$%&()*+,-./:;<=>?@[\\]^_`{|}~\t\n',
        ...
        return [word for word in input_text.split(split) if word]
...
def frequencyOfDigits(inputText):
    ...
    alldigits = re.findall(r'\d', inputText)
...
def frequencyOfSpecialCharacters(inputText):
    ...
    # inputText = inputText.lower().replace(" ", "")
    specialCharacters = open(_resource("writeprints_special_chars.txt"), "r").readlines()
...
def functionWordsPercentage(inputText):
    functionWords = open(_resource("functionWord.txt"), "r").readlines()
...
def frequencyOfPunctuationCharacters(inputText):
    ...
    inputText = str(inputText).lower()  # because its case insensitive
    inputText = inputText.lower().replace(" ", "")
    specialCharacters = open(_resource("writeprints_punctuation.txt"), "r").readlines()
    specialCharacters = [s.strip("\n") for s in specialCharacters]
    specialCharactersFrequencyDict = {}
    for c in range(0, len(specialCharacters)):
        specialChar = specialCharacters[c]
...
def misSpellingsPercentage(inputText):
    misspelledWords = open(_resource("writeprints_misspellings.txt"), "r").readlines()
